...
void *CNedAlloc::AllocateMemory(const size_t size)
{
	void *p = nedmalloc(size);

	if(!p)
		throw NOVA_EXP("nedmalloc: Can not allocate memory, return NULL pointer..", MEM_ERROR);

	return p;
}

void CNedAlloc::FreeMemory(void *pmem)
{
	nedfree(pmem);
}

void *CNedAlloc::Reallocate(void *pmem, const size_t size)
{
	void *p = nedrealloc(pmem, size);

	if(!p)
		throw NOVA_EXP("nedrealloc: Can not reallocate memory, return NULL pointer..", MEM_ERROR);
